// Pulled mostly from https://appventure.me/2018/01/10/optional-extensions/
...
public extension Optional {
    ...
    public func zip<A>(with other: Optional<A>) -> (Wrapped, A)? {
    ...
    /// Zips the content of this optional with the content of another
    /// optional `other` only if both optionals are not empty
    public func zip<A, B>(with other: Optional<A>, another: Optional<B>) -> (Wrapped, A, B)? {
    ...
    /// Zips the content of this optional with the content of another
    /// optional `other` only if both optionals are not empty
    public func zip<A, B, C>(with other: Optional<A>, third: Optional<B>, fourth: Optional<C>) -> (Wrapped, A, B, C)? {
        guard let first = self,
            let second = other,
            let third = third,
            let fourth = fourth
